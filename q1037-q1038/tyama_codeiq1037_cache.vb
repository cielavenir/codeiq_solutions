...
module CodeIQInfiniteProductVB
	iterator function infinite_product_process(of T)(ByVal arr as List(of List(of T)),ByVal result as List(of T),ByVal cnt as integer,ByVal d as integer) as IEnumerable(of List(of T))
		if d=arr.Count-1
			if 0<=cnt andalso cnt<arr(d).Count
				result.Add(arr(d)(cnt))
				yield result.ToList()
				result.RemoveAt(result.Count-1)
			end if
		else
			dim l as integer=Math.Min(arr(d).Count,cnt+1)
			for i as integer=0 to l-1
				result.Add(arr(d)(i))
				for each e as List(of T) in infinite_product_process(arr,result,cnt-i,d+1)
					yield e
				next
				result.RemoveAt(result.Count-1)
			next
	...
	iterator function infinite_product(of T)(ByVal iters as List(of IEnumerable(of T))) as IEnumerable(of List(of T))
		dim enums as List(of IEnumerator(of T))=iters.Select(Function(x) x.GetEnumerator()).ToList
		dim flags=new List(of boolean)
		for i as integer=0 to enums.Count-1
			flags.Add(true)
		next
		dim arr=new List(of List(of T))
		for i as integer=0 to enums.Count-1
			arr.Add(new List(of T))
		next
		...
		while not iter_end
			...
			for i as integer=0 to enums.Count-1
				if flags(i)
					flags(i)=enums(i).MoveNext()
				end if
				if flags(i)
					arr(i).Add(enums(i).Current)
				end if
			next
			for each e as List(of T) in infinite_product_process(arr,new List(of T),cnt,0)
				iter_end=false
				yield e
			next
			cnt+=1
