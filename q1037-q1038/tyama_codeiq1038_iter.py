...
import itertools
#注：Lv2ではIteratorClonerにおいてyield文を使用する要求があったが、今回はないので、素直にitertools.teeを使うこととする。

# いちいち回す解法。ただし、Pythonのclonerは事実上内部キャッシュなので、さほど影響はないのかもしれない。
...
def infinite_product(*iters):
	...
	while not iter_end:
		iter_end=True
		for e in infinite_product_process(enums,[],cnt,0):
			iter_end=False
			yield e
		cnt+=1
	# 桜先生からのワンポイントアドバイス：
...
def infinite_product_process(enums,result,cnt,d):
	...
	# ただし、以下の条件を満たすこと：
	# ・yield 式を必ず利用すること。
	enums[d],dup=itertools.tee(enums[d])
	for i in range(cnt+1):
		try:
			v=next(dup)
			if d<len(enums)-1:
				for e in infinite_product_process(enums,result+[v],cnt-i,d+1): yield e
			elif i==cnt:
				yield tuple(result+[v])
